...
async function getTransportBiltiMatch({ saleId, voucherNo = "", lorryNo = "" }) {
  ...
  if (mongoReady() && mongoose.connection?.db) {
    ...
    if (saleIdText) {
      mongoCandidates.push(
        collection.findOne({ sale_id: saleIdText }, { sort: { id: -1 } }),
        collection.findOne({ sale_id: saleIdText, payable_amount: { $exists: true } }, { sort: { id: -1 } })
      );
    }
    if (voucherNoText) mongoCandidates.push(collection.findOne({ voucher_no: voucherNoText }, { sort: { id: -1 } }));
    if (lorryNoText) mongoCandidates.push(collection.findOne({ lorry_no: lorryNoText }, { sort: { id: -1 } }));

    for (const promise of mongoCandidates) {
      const doc = await promise;
      if (doc) return { ...doc, transport_amount: Number(doc.payable_amount || doc.net_amount || doc.gross_freight || 0), source: "mongo" };
    }
  }
