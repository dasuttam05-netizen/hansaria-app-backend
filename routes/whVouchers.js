...
function buildPurchasePayload(body, voucherNo) {
  ...
  const netPayable = purchaseNetPayableFromRow(payload);
  payload.amount = netPayable;
  payload.net_amount = netPayable;
...
// Canonical purchase amount used by Purchase Summary, Pending Amount and Payment Adjustment.
function purchaseNetPayableFromRow(row = {}) {
  const gross = purchaseGrossAmountFromRow(row);
  const deduction = purchaseDeductionTotalFromRow(row);
    ...
    Promise.all([
      ...
      .then(([purchaseRows, paymentRows]) => {
        const totalPurchase = (purchaseRows || []).reduce(
          (sum, row) => sum + purchaseNetPayableFromRow(row),
          0
        );
    ...
    const resolveWithMongo = async () => {
      ...
      for (const item of cleanAdjustments) {
        ...
        if (purchase) {
          ...
          const billAmount = purchaseNetPayableFromRow(purchase);
    ...
    getPaymentAdjustmentsByPurchase((adjustErr, adjustedMap) => {
        ...
        const send = (purchaseRows) => {
          const purchases = (purchaseRows || []).map((row) => {
            ...
            const amount = purchaseNetPayableFromRow(row);
            const adjusted_amount = Number((adjustedMap.get(purchaseId) || 0).toFixed(2));
            return {
              ...row,
              id: purchaseId,
              amount,
        ...
        if (mongoReady()) {
          ...
          PurchaseVoucher.find(filter)
            ...
            .then((rows) => send(rows.map((row) => ({
              ...row,
              amount: purchaseNetPayableFromRow(row),
              net_amount_payable: purchaseNetPayableFromRow(row),
            }))))
          ...
          if (persistedPurchaseDeductionUpdates.length) {
            ...
            const nextLinks = currentLinks.map((link) => {
              ...
              return {
                ...link,
                purchase_id: purchaseId,
                claim_amount: update.final.claim,
                bags_claim: update.final.claim,
                labour: update.final.labour,
                transport_charge: update.final.freight,
                cd_amount: update.final.cashDiscount,
                tds_amount: update.final.tds,
                other_deduction: update.final.other,
...
router.get("/payment/:id/pdf", async (req, res) => {
  if (!userHasPermission(req.user, "warehouse.trading.payment.view")) {
    return res.status(403).json({ error: "Permission denied" });
  }
  try {
    ...
    const refs = [];
    rawAdjustments.forEach(a => {
      if (a.purchase_id != null) refs.push(String(a.purchase_id));
      if (a.voucher_no) refs.push(String(a.voucher_no).trim());
    });
    const reference = String(row.reference_id || row.reference || "").trim();
    if (reference) refs.push(reference);

    const purchases = [];
    if (PurchaseVoucher && refs.length) {
      const or = [];
      const objectIds = refs.filter(v => mongoose.Types.ObjectId.isValid(v));
      const numericIds = refs.map(Number).filter(Number.isFinite);
      if (objectIds.length) or.push({ _id: { $in: [...new Set(objectIds)] } });
      if (numericIds.length) or.push({ id: { $in: [...new Set(numericIds)] } });
      const escaped = reference.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      if (reference) {
        or.push({ voucher_no: reference }, { bill_no: reference });
        or.push({ voucher_no: { $regex: `^\\s*${escaped}\\s*$`, $options: "i" } });
    ...
    purchases.forEach(p => {
      if (p._id != null) byId.set(String(p._id), p);
      if (p.id != null) byId.set(String(p.id), p);
      if (p.voucher_no) byVoucher.set(String(p.voucher_no).trim().toLowerCase(), p);
      if (p.bill_no) byVoucher.set(String(p.bill_no).trim().toLowerCase(), p);
    });

    const billRows = rawAdjustments.map(a => ({
      purchase: byId.get(String(a.purchase_id || "")) || byVoucher.get(String(a.voucher_no || "").trim().toLowerCase()),
      adjusted: Number(a.adjusted_amount || 0)
    })).filter(x => x.purchase);
    if (!billRows.length && reference) {
      const p = byVoucher.get(reference.toLowerCase());
      if (p) billRows.push({ purchase: p, adjusted: Number(row.amount || 0) });
    }
    ...
    const amountWords = (value) => {
      ...
      let n=Math.floor(Math.abs(Number(value)||0)); if(!n)return "Zero Rupees Only";
      ...
      if(crore)parts.push(`${under1000(crore)} Crore`); if(lakh)parts.push(`${under100(lakh)} Lakh`); if(thousand)parts.push(`${under100(thousand)} Thousand`); if(n)parts.push(under1000(n));
      ...
      purchaseLinks.map(async (link) => {
        ...
        try {
          ...
          return {
            ...
            purchase_deduction_manual_modes: purchase.sale_summary_manual_modes || link.purchase_deduction_manual_modes || link.sale_summary_manual_modes || {},
            purchase_deduction_final: link.purchase_deduction_final || {},
          };
        } catch {
          return link;
        }
